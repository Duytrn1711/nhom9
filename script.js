...
const renderProducts = () => {
    ...
    if (searchQuery) {
        ...
        if (sectionTitle) {
            ...
            sectionTitle.textContent = isCategoryPage ? "Tất cả sản phẩm" : "Sản phẩm bán chạy";
            document.title = isCategoryPage ? "Sản phẩm - ShopVip" : "ShopVip - Cửa Hàng Hiện Đại";
        }
    }
...
const showSearchDropdown = (input, dropdown, query) => {
    ...
    if (matchedProducts.length === 0) {
        ...
        dropdown.innerHTML = matchedProducts.map(product => `
            <div class="search-result-item" onclick="window.location.href='category.html?q=${encodeURIComponent(product.name)}'">
